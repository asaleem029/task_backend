...
const ormExceptionHandling = (error: any): IError[] | IError => {
  CustomLogger.log('error', 'Error through OrmErrorHandling: ' + JSON.stringify(error));
  ...
  CustomLogger.log('error', 'Error through OrmErrorHandling: ' + JSON.stringify(errorObjects));
  return errorObjects;
};
