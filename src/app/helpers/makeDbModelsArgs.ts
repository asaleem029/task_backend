import { SelectQueryBuilder, EntityTarget } from 'typeorm';
...
const makeQueryBuilder = (
  ...
  queryBuilderFunction: (rawCriteria: any, alias: string) => any,
  ...
  const queryBuilder = DBConn.getRepository(entity).createQueryBuilder();
  const { params, query } = queryBuilderFunction(args, queryBuilder.alias);
  ...
  if (select && select.length) {
    const selectColumns = select.split(',').map((col: string) => `${queryBuilder.alias}.${col}`);
  ...
  if (relations && relations.length) {
    relations.split(',').forEach((relation: string) => {
      ...
      if (relationsQueryBuilderFunctions) {
        const { params: childparams, query: childquery } = relationsQueryBuilderFunctions[
          relation
        ].method(relationsQueryBuilderFunctions[relation].query, relation);
  ...
  if (childQuery && childParams) queryBuilder.andWhere(childQuery, childParams);
  return queryBuilder;
};

const makeQueryBuilderGetOne = (
  { select, relations, ...args }: any,
  queryBuilderFunction: (rawCriteria: any, alias: string) => any,
  entity: EntityTarget<any>,
  relationsQueryBuilderFunctions?: IChildQuery,
): SelectQueryBuilder<any> => {
